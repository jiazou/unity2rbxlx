# Scene-runtime architecture — execution log

**Status:** execution / status companion to [`scene-runtime-architecture-ir.md`](scene-runtime-architecture-ir.md)
(the architecture-of-record). This file holds the per-slice ledgers, per-phase testing specs, sequencing,
and the chronological revision history that were split out of the architecture doc on 2026-06-06 so the
architecture stays readable. The durable DESIGN substance (the topology artifact schema, producer/consumer
boundary, persistence rule, risks, open questions) lives in the parent doc; the verifier check specs and
the two-bridge-class distinction live HERE inside the phase plan (they're phase deliverables). This is the
WHAT-SHIPPED-WHEN record; entries reference the parent doc's sections by name.
...
1. **`infer_module_domains` misclassifies Roblox-dead Unity rendering
   ...
   the "Roblox-dead module handling" section in `scene-runtime-architecture-ir.md` + the brief. (Task #8.)
...
1. **New `scene_runtime_topology/` package** with the 5 sub-modules
   sketched in `scene-runtime-architecture-ir.md` §Architecture. Most of the logic in today's `scene_runtime_domain.py`
...
4. **Lightweight invariant checks at emit time** — the five invariants
   listed under "The topology artifact" in `scene-runtime-architecture-ir.md`. Abort the build on violation.
...
- **Slice 6** (shipped, PR #152): pipeline split — extracted
  ...
  "Persistence rule" section in `scene-runtime-architecture-ir.md`.
- **Slice 7** (shipped): `_decide_script_container` rewrite consuming
  `topology_inputs`. This is where the corrected decision tree (in `scene-runtime-architecture-ir.md`)
  became live code.
...
1. **Rewrite `_decide_script_container` to read topology facts.** The
   ...
   in `test_scene_runtime_domain.py`. The decision tree in `scene-runtime-architecture-ir.md` replaces
...
|---|---|---|
| 1 (#159) | Edge schema. **Revise:** drop `SHARED_ATTRIBUTE_SEEDS` / `producer_domain` / the locked `PickupItemEvent` (all artifacts of mis-modeling Class 2 as Class 1). KEEP the `CrossDomainEdge` schema for Class 1. | **shipped** (reframe landed, upstream/main 2026-06-01) |
| 2 (#160) | Enrichment + two buckets. **KEEP** the component-ref edge path + the reader scan (Class 2's one reusable piece). **Replace** the shared-flag candidate bucket with the `shared_flag_channels` fact; fix the persistence-drift. | **shipped** (upstream/main 2026-06-01/02) |
