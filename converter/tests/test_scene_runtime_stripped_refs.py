...
# ---------------------------------------------------------------------------
# Fix #2 -- late unresolved-WARN path: a stripped ref whose placed target is
# itself UI-DEFERRED and NEVER registers (its host clone never lands). After
# start() + the deferred batch (_completeDeferredBatch -> _drainPendingPlacementRefs)
# the field stays nil and the residual WARN fires (no crash). This exercises the
# drain call site INSIDE _completeDeferredBatch (the AC5 test instead has a
# subplan with no instance, so the target never defers).
# ---------------------------------------------------------------------------

class TestLateUnresolvedWarnWhenDeferredTargetNeverLands:
    ...
    def _scenario(self) -> str:
        return textwrap.dedent("""\
            ...
            function Loadout.new(_) return setmetatable({}, Loadout) end
            function Loadout:Start()
                assert(self.missionPopup == nil,
                    "deferred target that never landed => nil")
            end
            -- Placed UI-owned prefab whose host clone NEVER lands -> the placed
            -- instance DEFERS during the placement boot loop, then its
            -- _completeDeferredBatch sees goInst==nil, never builds/registers the
            -- component, and the drain at the end of the batch leaves the field
            -- nil + WARN.
            local PLACEMENT = "Main:9"
            local PREFAB = "guidU:Assets/Prefabs/UI/MissionPopup.prefab"
            local SRC_FID = "222"
            local PREFAB_LOCAL = PREFAB .. ":" .. SRC_FID
            local ENGINE_KEY = PLACEMENT .. ":" .. PREFAB_LOCAL
            local plan = {
            ...
            -- Only the scene source exists -> the placed UI instance has no
            -- clone at boot and DEFERS. ``awaitUiHost`` returns nil for it -> the
            -- host clone never lands -> the deferred batch never builds it.
            local instances = {
                sgo = {Name = "Loadout", _sceneRuntimeId = "sgo", _children = {}},
            }
            local services = servicesFor(plan, {loadout = Loadout, missionui = MissionUI}, instances)
            ...
            local engine = SceneRuntime.new(services, plan)
            engine:start("client")
            runDeferred()  -- flush deferred Starts + the late UI batch

            local src = engine._componentByInstanceId["Main:1"]
            assert(src.missionPopup == nil, "field must stay nil end-to-end")
            -- The placed component was never registered (its host never landed).
            assert(engine._componentByInstanceId[ENGINE_KEY] == nil,
                "deferred target must NOT be registered")
            local warned = false
            for _, line in ipairs(logs) do
                if string.find(line, "stripped%-prefab ref")
                   and string.find(line, "never") then warned = true end
            end
            assert(warned,
                "late drain (inside _completeDeferredBatch) must WARN once the "
                .. "deferred target is known absent")
            print("OK")
            print("DONE")
        """)

    def test_deferred_target_never_lands_warns_no_crash(self):
        rc, out, err = _run_scenario(self._scenario())
        assert rc == 0, f"luau failed: {err}\n{out}"
        assert "OK" in out, out
...
# ---------------------------------------------------------------------------
# Fix #6 -- the residual-WARN gate must NOT fire prematurely across multiple
# deferred host GROUPS. _resolveDeferredUiInstances clears _deferredUiInstances
# ONCE up front then completes one group per _completeDeferredBatch. If group A
# (a peer) completes before group B (which hosts the placement target), the drain
# after group A must NOT WARN "never resolved" -- group B then registers the
# target and a later drain binds it. The fix gates the WARN on
# _deferredGroupsPending (groups still in flight), not on _deferredUiInstances
# (already nil'd up front).
# ---------------------------------------------------------------------------

class TestResidualWarnNotPrematureAcrossDeferredGroups:

    def _scenario(self, *, break_group_gate: bool) -> str:
        # ``break_group_gate`` models the PRE-FIX gate: the residual WARN fired
        # whenever _deferredUiInstances was nil (which it is, up front), ignoring
        # groups still in flight. We simulate that by zeroing the in-flight
        # counter right after start() begins the deferred resolution is not
        # directly toggleable; instead we shadow the per-group drain so the FIRST
        # group's completion runs the OLD (counter-blind) gate. Simpler + robust:
        # disable the counter entirely by stubbing it to nil before the deferred
        # batches run, reproducing the pre-fix premature-fire.
        red = (
            "engine._deferredGroupsPending = nil  -- pre-fix: counter-blind gate\n"
            if break_group_gate else ""
        )
        # Two placements of UI-owned prefabs, hosted on DIFFERENT GOs (so they
        # form two deferred GROUPS). The scene source's ref targets the prefab in
        # group B. Group A (a peer placement on its own host) resolves FIRST and
        # its drain must not WARN while group B is still pending.
        return textwrap.dedent("""\
            local Src = {} ; Src.__index = Src
            function Src.new(_) return setmetatable({}, Src) end
            local Tgt = {} ; Tgt.__index = Tgt
            function Tgt.new(_) return setmetatable({mark="TGT"}, Tgt) end
            function Tgt:Awake() assert(self.gameObject ~= nil, "Tgt.go bound") end
            local Peer = {} ; Peer.__index = Peer
            function Peer.new(_) return setmetatable({mark="PEER"}, Peer) end
            function Peer:Awake() assert(self.gameObject ~= nil, "Peer.go bound") end

            local TGT_KEY = "Main:9:pfbT:222"   -- scene ref target (group B)
            ...
            local plan = {
                ...
                scenes = {
                    Main = {
                        ...
                        references = {
                            {["from"] = "Main:1", field = "tgtRef", index = nil,
                             target_kind = "component", target_ref = TGT_KEY,
                             target_script_id = "tgt", target_is_ui = false},
                        },
                        lifecycle_order = {"Main:1"},
                    },
                },
                prefabs = {
                    ["pfbT"] = {
                        name = "Tgt",
                        instances = {{instance_id = "pfbT:222", script_id = "tgt",
                                      game_object_id = "pfbT:host", active = true,
                                      enabled = true, config = {},
                                      instance_owner_is_ui = true}},
                        references = {}, lifecycle_order = {"pfbT:222"},
                    },
                    ["pfbP"] = {
                        name = "Peer",
                        instances = {{instance_id = "pfbP:333", script_id = "peer",
                                      game_object_id = "pfbP:host", active = true,
                                      enabled = true, config = {},
                                      instance_owner_is_ui = true}},
                        references = {}, lifecycle_order = {"pfbP:333"},
                    },
                },
                scene_prefab_placements = {
                    {placement_id = "Main:9", prefab_id = "pfbT",
                     active = true, enabled = true},
                    {placement_id = "Main:8", prefab_id = "pfbP",
                     active = true, enabled = true},
                },
                domain_overrides = {},
            }
            ...
            local services = servicesFor(plan, {src = Src, tgt = Tgt, peer = Peer}, instances)
            -- Both placed hosts land LATE (during the deferred batch). The peer
            -- group (Main:8) is grouped under its own host goid, distinct from the
            -- target group (Main:9) -> two groups.
            local tgtClone = {Name = "TgtHost", _sceneRuntimeId = "Main:9:pfbT:host",
                              _children = {}}
            local peerClone = {Name = "PeerHost", _sceneRuntimeId = "Main:8:pfbP:host",
                               _children = {}}
            services.awaitUiHost = function(id)
                if id == "Main:9:pfbT:host" then return tgtClone end
                if id == "Main:8:pfbP:host" then return peerClone end
                return nil
            end
            ...
            local src = engine._componentByInstanceId["Main:1"]
            assert(src ~= nil, "source must exist")

            -- The target IS eventually bound (the drain binds it once group B
            -- completes) -- binding behaviour is unchanged either way.
            if type(src.tgtRef) == "table" and src.tgtRef.mark == "TGT" then
                print("TARGET_BOUND")
            end

            -- A premature/spurious "never resolved" WARN for tgtRef is the bug.
            local spuriousWarn = false
            for _, line in ipairs(logs) do
                if string.find(line, "stripped%-prefab ref")
                   and string.find(line, "tgtRef")
                   and string.find(line, "never") then spuriousWarn = true end
            end
            if spuriousWarn then print("SPURIOUS_WARN") else print("NO_SPURIOUS_WARN") end
            print("DONE")
        """).replace("__RED__", red)

    def test_no_premature_warn_when_target_in_later_group(self):
        # GREEN: with the in-flight group counter, the early group's drain does
        # not WARN; the target binds and no spurious "never resolved" fires.
        rc, out, err = _run_scenario(self._scenario(break_group_gate=False))
        assert rc == 0, f"luau failed: {err}\n{out}"
        assert "TARGET_BOUND" in out, (
            f"the placement target must bind once its group completes; got:\n{out}")
        assert "NO_SPURIOUS_WARN" in out, (
            f"the residual WARN must NOT fire while a later deferred group can "
            f"still register the target; got:\n{out}")

    def test_red_counter_blind_gate_fires_premature_warn(self):
        # RED: with the in-flight counter disabled (pre-fix gate) the FIRST
        # group's drain WARNs "never resolved" even though a later group binds it.
        rc, out, err = _run_scenario(self._scenario(break_group_gate=True))
        assert rc == 0, f"luau failed: {err}\n{out}"
        assert "SPURIOUS_WARN" in out, (
            f"RED proof: a counter-blind gate fires a premature WARN across "
            f"deferred groups; got:\n{out}")
