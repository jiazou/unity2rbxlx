...
def _make_ui_element(parent_xml: ET.Element, elem: RbxUIElement) -> None:
    ...
    # Serialize attributes (e.g., _SceneRuntimeId, widget config).
...
def write_rbxlx(place: RbxPlace, output_path: Path) -> dict[str, int]:
    ...
    # Button onClick wiring is carried by the ``ClickBinding`` round-trip
    # (build-time emit -> ``scene_runtime["ui_click_bindings"]`` -> the host
    # runtime's ``_installClickWatch``), NOT a print-stub LocalScript. Nothing
    # to emit here.
