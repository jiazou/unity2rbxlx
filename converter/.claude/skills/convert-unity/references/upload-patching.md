...
3. `convert_animations` — `converter/animation_converter.py` converts transform-only `.anim` clips into inline TweenService Luau scripts. Humanoid/skeletal clips and `AnimatorController` state machines are unsupported and surfaced to `UNCONVERTED.md` (see `docs/UNSUPPORTED.md`). Runtime modules under `runtime/` are *conditionally* injected based on what the scene actually uses: `nav_mesh_runtime.luau`, `event_system.luau`, `physics_bridge.luau`, `cinemachine_runtime.luau`, `sub_emitter_runtime.luau`. The old `pickup_runtime.luau` is no longer injected — pickup scripts are propagated directly from base prefabs to variants in `_bind_scripts_to_parts`.
