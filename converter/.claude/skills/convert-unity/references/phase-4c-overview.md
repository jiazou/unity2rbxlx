...
**No genre-specific scaffolding.** PR8 retired the `--scaffolding=fps` injection path; FPS controllers, HUD ScreenGuis, and weapon-mount helpers are no longer generated by the converter. The retired emitters are not replaceable by an override knob — `scene_runtime.domain_overrides` only pins per-MonoBehaviour-class storage/domain placement, it does not generate new scripts. If a project needs an FPS controller or HUD on the converted output, port the equivalent Unity behavior into the source project so the AI transpiler emits it, or hand-author the LocalScript and place it via `storage_plan` (see `phase-4a-storage-classification.md` for the override surfaces and their current sticky semantics).
